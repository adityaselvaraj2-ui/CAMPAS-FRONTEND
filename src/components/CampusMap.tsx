...
const CampusMap = ({ campus, onBack }: CampusMapProps) => {
  ...
  useEffect(() => {
    ...
    const map = L.map(mapRef.current, {
      ...
      zoom: campus.id === 'cit' || campus.id === 'kpr' ? 16 : 17,
      zoomControl: true,
    ...
    campus.buildings.forEach((building) => {
      ...
      const icon = L.divIcon({
        ...
        html: `
          ...
          <div class="pulse-ring beacon-ring beacon-ring-delay-2" style="border-color: ${campus.color}10;"></div>
  ...
  return (
    <div
      className="relative w-full h-[calc(100vh-70px)] tab-enter"
      style={{ animation: 'map-iris-open 900ms cubic-bezier(0.16,1,0.3,1) 200ms both' }}
    >
      {/* Top bar */}
      <div className="absolute top-4 left-4 right-4 z-[1000] flex items-center justify-between">
        <button
          onClick={onBack}
          className="glass-card px-4 py-2 font-ui text-xs tracking-wider text-muted-foreground hover:text-primary hover:-translate-y-0.5 active:translate-y-0 transition-all"
        >
          ← BACK
        </button>
        ...
        <button
          ...
          className="glass-card px-4 py-2 font-ui text-xs tracking-wider text-muted-foreground hover:text-primary transition-all"
        >
