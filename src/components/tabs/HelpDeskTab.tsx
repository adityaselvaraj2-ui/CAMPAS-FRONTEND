...
const HelpDeskTab = ({ campus }: HelpDeskTabProps) => {
  ...
  const handleSend = async (text?: string) => {
    ...
    try {
      const { reply } = await sendChatMessage(msg, campus || '');
        ...
        <div className="flex-1 lg:w-[55%] glass-card p-4 flex flex-col" style={{ minHeight: '500px', maxHeight: '70vh' }}>
          ...
          <div className="flex-1 overflow-y-auto space-y-3 mb-4 pr-1">
            ...
            {isLoading && (
              <div className="flex justify-start animate-fade-up">
                <div className="px-4 py-3 rounded-xl glass border border-border text-text-3 text-sm">
                  Thinking...
                </div>
              </div>
            )}
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="h-12 w-12 rounded-xl bg-primary/20 border border-primary/30 flex items-center justify-center text-primary hover:bg-primary/30 hover:-translate-y-0.5 active:translate-y-0 transition-all disabled:opacity-50"
            >
